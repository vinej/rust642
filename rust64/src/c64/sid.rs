...
#[cfg(not(target_os = "redox"))]
use self::sdl3::audio::{ AudioCallback, AudioFormat, AudioSpec, AudioStream };
use c64::memory;
use c64::sid_tables::*;
...
// the SID chip with associated SDL3 audio stream
pub struct SID {
    mem_ref: Option<memory::MemShared>,
    #[cfg(not(target_os = "redox"))]
    audio_device: sdl3::audio::AudioStreamWithCallback<SIDAudioDevice>,
}
...
impl SID {
    pub fn new_shared() -> SIDShared {
        ...
        let desired_spec = AudioSpec {
            ...
            format:   Some(AudioFormat::s16_sys()),     // signed 16-bit native-endian
        };
        println!("{:?}", desired_spec);

        let audio_device = audio_subsystem
            .open_playback_stream(&desired_spec, SIDAudioDevice::new())
            .unwrap();
    ...
    pub fn reset(&mut self) {
        let mut lock = self.audio_device.lock().unwrap();
        (*lock).reset();
    }
    ...
    pub fn update_audio(&mut self) {
        let _ = self.audio_device.resume();
    }
}
...
// SDL3 audio callback implementation - this is where the samples are being converted to output sound.
// SDL3 hands us a stream and a requested sample count; we fill a local buffer and push it.
#[cfg(not(target_os = "redox"))]
impl AudioCallback<i16> for SIDAudioDevice {
    fn callback(&mut self, stream: &mut AudioStream, requested: i32) {
        let n = requested.max(0) as usize;
        let mut buf: Vec<i16> = vec![0; n];
        self.render(&mut buf);
        let _ = stream.put_data_i16(&buf);
    }
}
